...
//杨飞添加 
				if(!empty($segment_q)) {
					$sql = "`siteid`= '$siteid' AND `typeid` = '$typeid' $catid_diy $sql_time AND `data` like '%$q%'";
				//	$sql = "`siteid`= '$siteid' AND `typeid` = '$typeid' $catid_diy $sql_time AND MATCH (`data`) AGAINST ('$segment_q' IN BOOLEAN MODE)";
					}else{
						$sql = "`siteid`= '$siteid' AND `typeid` = '$typeid' $catid_diy $sql_time AND `data` like '%$q%'";
						}
